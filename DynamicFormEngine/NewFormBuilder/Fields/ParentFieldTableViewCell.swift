...
class ParentFieldTableViewCell: UITableViewCell {
    ...
    weak var delegate: ParentFieldTableViewCellDelegate?
...
//            nestedVC.changeErrorValue(isError: error, section: attachmentView.indexPath.section, row: attachmentView.indexPath.row)
    ...
    func showToolTip(toolTip: String, sender: Any) {
        if let formBaseVC = formViewController as? FormBaseViewController {
            formBaseVC.showToolTip(toolTip: toolTip, sender: sender)
        }else if let summaryVC = formViewController as? ServiceSummaryPagesVC {
            summaryVC.showToolTip(toolTip: toolTip, sender: sender)
        }
    }
