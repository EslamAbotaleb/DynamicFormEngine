...
class FormManager {
    ...
    func fetchOptionsList(vc: FormBaseViewController, sectionObjects: [SectionObject], code: String, section: Int, row: Int, isLoadCell: Bool = false, completion: (([MCQOption]) -> Void)? = nil) {
        ...
        DispatchQueue.global(qos: .background).async { [weak self] in
            ...
            disposable = cerqel_BasicNetworkServiceImpl.shared.load(cerqel_CodableResponseObjectDynamicForm<SearchDynamicModel>(action: Dynamic_BasicAction.fetchDDLOptions(code: code, targetComponents: vc.formBuilder.targetComponents.value)))
    ...
    func fetchDataSourceOptionsList(vc: FormBaseViewController, sectionObjects: [SectionObject], submitButton: UIButton, code: String, parameters: [[String:String]], section: Int, row: Int, isLoadCell: Bool = false, completion: (([MCQOption]) -> Void)? = nil) {
        ...
        cerqel_BasicNetworkServiceImpl.shared.load(cerqel_CodableResponseObjectDynamicForm<SearchDynamicModel>(action: Dynamic_BasicAction.fetchDataSourceOptions(code: code, parameters: params, targetComponents: targetComps))).subscribe(onNext: {[weak self] (response) in
            guard let `self` = self else {return}
            self.cerqel_HUD.stopHUD()
            var sectionIndex = 0
    ...
    func fetchCascadingOptionsList(vc: FormBaseViewController, sectionObjects: [SectionObject], code: String,parentValue: [String:String]?, section: Int, row: Int) {
        ...
        cerqel_BasicNetworkServiceImpl.shared.load(cerqel_CodableResponseObjectDynamicForm<SearchDynamicModel>(action: Dynamic_BasicAction.fetchCascadingOptions(code: code, parentValue: parentValue ?? [:], targetComponents: targetComps))).subscribe(onNext: {[weak self] (response) in
            guard let `self` = self else {return}
            self.cerqel_HUD.stopHUD()
            if let data = response.item?.data {
                if let components = data.components {
    ...
    func fetchCascadingOptionsListWithMultiParents(vc: FormBaseViewController, sectionObjects: [SectionObject], submitButton: UIButton, code:String, parameters:[[String:String]], row: Int) {
        ...
        cerqel_BasicNetworkServiceImpl.shared.load(cerqel_CodableResponseObjectDynamicForm<SearchDynamicModel>(action: Dynamic_BasicAction.fetchCascadingOptionsWithMultiParents(code: code, parameters: params, targetComponents: targetComps))).subscribe(onNext: {[weak self] (response) in
            guard let `self` = self else {return}
            self.cerqel_HUD.stopHUD()
            var sectionIndex = 0
            var isFromTable = false
    ...
    func fetchSearchOptions(vc: FormBaseViewController, sectionObjects: [SectionObject], code: String,keyword: String, params: [String:String]?, section: Int, row: Int, completion: @escaping (([MCQOption]) ->Void)) {
        ...
        cerqel_BasicNetworkServiceImpl.shared.load(cerqel_CodableResponseObjectDynamicForm<SearchDynamicModel>(action: Dynamic_BasicAction.fetchSearchOptions(code: code,
                                                                                                                                                   keyword: keyword,
                                                                                                                                                   parameters: params,
