...
//        SubmittedSuccessRequestBottomSheet.open(presenter: self,data: successData, fromDetails: false)
    ...
    func fetchSearchOptions(code: String,keyword: String, params: [String:String]?, section: Int, row: Int, completion: @escaping (([MCQOption]) ->Void)) {
        ...
        cerqel_BasicNetworkServiceDynamicFormImpl.shared.load(cerqel_CodableResponseObject<SearchDynamicModel>(action: cerqel_BasicActionDynamicForm.fetchSearchOptions(code: code,
                                                                                                                                                   keyword: keyword,
...
//        router.popToViewController(ofClass: AllServicesViewController.self, animated: true)
    ...
    func fetchCascadingOptionsListWithMultiParents(code:String, parameters:[[String:String]], row: Int) {
        cerqel_HUD.showHUD()
        let targetComps =  getTargetComponents(for: code, row: row)
        cerqel_BasicNetworkServiceDynamicFormImpl.shared.load(cerqel_CodableResponseObject<SearchDynamicModel>(action: cerqel_BasicActionDynamicForm.fetchCascadingOptionsWithMultiParents(code: code, parameters: parameters, targetComponents: targetComps))).subscribe(onNext: {[weak self] (response) in
            guard let `self` = self else {return}
            self.cerqel_HUD.stopHUD()
            var sectionIndex = 0
...
//                                    self.handleRequiredItems(section: section)
    ...
    func fetchCascadingOptionsList(code: String,parentValue: [String:String]?, section: Int, row: Int) {
        cerqel_HUD.showHUD()
        let targetComps =  getTargetComponents(for: code, row: row)
        cerqel_BasicNetworkServiceDynamicFormImpl.shared.load(cerqel_CodableResponseObject<SearchDynamicModel>(action: cerqel_BasicActionDynamicForm.fetchCascadingOptions(code: code, parentValue: parentValue ?? [:], targetComponents: targetComps))).subscribe(onNext: {[weak self] (response) in
            guard let `self` = self else {return}
            self.cerqel_HUD.stopHUD()
            if let data = response.item?.data {
                if let components = data.components {
    ...
    func fetchOptionsList(code: String, section: Int, row: Int, isLoadCell: Bool = false, completion: (([MCQOption]) -> Void)? = nil) {
        ...
        DispatchQueue.global(qos: .background).async { [weak self] in
            ...
            disposable = cerqel_BasicNetworkServiceDynamicFormImpl.shared.load(cerqel_CodableResponseObject<SearchDynamicModel>(action: cerqel_BasicActionDynamicForm.fetchDDLOptions(code: code, targetComponents: self.formBuilder.targetComponents.value)))
